...
constexpr const char* EXPERIMENTER_END_DISCARD_REPRESENTATION = "End Discard";
#define EXPERIMENTER_END_DISCARD_VALUE EXPERIMENTER_NAME
#define EXPERIMENTER_END_DISCARD_EXPLANATION (    \
	"This player discards all cards named "       \
	+ effect_value                                \
	+ " from their hand at the end of this turn." \
)
...
constexpr const char* BANKER_END_DISCARD_REPRESENTATION = "End Discard";
#define BANKER_END_DISCARD_VALUE BANKER_NAME
#define BANKER_END_DISCARD_EXPLANATION (          \
	"This player discards all cards named "       \
	+ effect_value                                \
	+ " from their hand at the end of this turn." \
)
...
constexpr int PLAYER_EXPLANATION_COUNT = 14;
...
#define PLAYER_EFFECT_CONDITIONS(X) (                                 \
	...
	: X == 2 ? static_cast<bool>(ATTACKED_CONDITION)                  \
	: X == 3 ? static_cast<bool>(RETREATED_CONDITION)                 \
	: X == 4 ? static_cast<bool>(FORBIDDEN_FUEL_CONDITION)            \
	: X == 5 ? static_cast<bool>(BOOST_CONDITION)                     \
	: X == 6 ? static_cast<bool>(BANISHMENT_CONDITION)                \
	: X == 7 ? static_cast<bool>(FREEDOM_CONDITION)                   \
	: X == 8 ? static_cast<bool>(AGILITY_AURA_CONDITION)              \
	: X == 9 ? static_cast<bool>(HEAL_AURA_CONDITION)                 \
	: X == 10 ? static_cast<bool>(PLAYER_POWER_CONDITION)             \
	: X == 11 ? static_cast<bool>(END_DRAW_CONDITION)                 \
	: X == 12 ? static_cast<bool>(EXPERIMENTER_END_DISCARD_CONDITION) \
	: static_cast<bool>(BANKER_END_DISCARD_CONDITION)                 \
)
//}
...
constexpr const char* PLAYER_EFFECT_REPRESENTATIONS[PLAYER_EXPLANATION_COUNT] = {
	...
	EXPERIMENTER_END_DISCARD_REPRESENTATION,
	BANKER_END_DISCARD_REPRESENTATION
};
...
#define PLAYER_EFFECT_VALUES(X) (              \
	...
	: X == 2 ? ATTACKED_VALUE                  \
	: X == 3 ? RETREATED_VALUE                 \
	: X == 4 ? FORBIDDEN_FUEL_VALUE            \
	: X == 5 ? BOOST_VALUE                     \
	: X == 6 ? BANISHMENT_VALUE                \
	: X == 7 ? FREEDOM_VALUE                   \
	: X == 8 ? AGILITY_AURA_VALUE              \
	: X == 9 ? HEAL_AURA_VALUE                 \
	: X == 10 ? PLAYER_POWER_VALUE             \
	: X == 11 ? END_DRAW_VALUE                 \
	: X == 12 ? EXPERIMENTER_END_DISCARD_VALUE \
	: BANKER_END_DISCARD_VALUE                 \
)
//}
...
#define PLAYER_EFFECT_EXPLANATIONS(X) (              \
	...
	: X == 2 ? ATTACKED_EXPLANATION                  \
	: X == 3 ? RETREATED_EXPLANATION                 \
	: X == 4 ? FORBIDDEN_FUEL_EXPLANATION            \
	: X == 5 ? BOOST_EXPLANATION                     \
	: X == 6 ? BANISHMENT_EXPLANATION                \
	: X == 7 ? FREEDOM_EXPLANATION                   \
	: X == 8 ? AGILITY_AURA_EXPLANATION              \
	: X == 9 ? HEAL_AURA_EXPLANATION                 \
	: X == 10 ? PLAYER_POWER_EXPLANATION             \
	: X == 11 ? END_DRAW_EXPLANATION                 \
	: X == 12 ? EXPERIMENTER_END_DISCARD_EXPLANATION \
	: BANKER_END_DISCARD_EXPLANATION                 \
)
//}
//}
//}
...
constexpr const char* PYROMANCER_ABILITY_DESCRIPTION =
	"Once a turn, you may discard the top 5 cards of each player's deck."
;
...
constexpr const char* SCUBA_DIVER_ATTACK_DESCRIPTION =
	"Deal 500 damage to your opponent's active fighter.\n"
	"If this fighter has the invinciblity effect, deal 500 more damage.\n"
...
constexpr const char* PYROTECHNICIAN_ATTACK_DESCRIPTION =
	"Deal 0.3 damage to your opponent's active fighter "
...
constexpr int BANISHER_HEALTH = 1300;
...
constexpr const char* CULTIST_ABILITY_DESCRIPTION =
	"Once a turn, you may heal 15 damage from one "
...
constexpr const char* NINJA_ATTACK_DESCRIPTION =
	"Flip 6 coins.\n"
	"For each heads, deal 175 damage to one of your opponent's fighters for each heads."
;
...
constexpr const char* SAMURAI_ATTACK_DESCRIPTION =
	"Deal 300 damage to your opponent's active fighter.\n"
	"Flip a coin.\n"
	"If heads, your opponent's active fighter can't "
...
constexpr const char* DOCTOR_DESCRIPTION =
	...
	"Heal 200 damage from each of your fighters for each card discarded."
;
...
constexpr int SUPPORTER_COUNT = 34;
...
class Player: public Affectable {
		...
		void attack(bool combo = false) noexcept {
			...
			for (int i = 0; i < effects.size(); ++i) {
				...
				// The effect is only resolved if heads was flipped.
					...
					// Cripples the opponent's active fighter (no retreat).
					else if (effects[i][2] == CRIPPLE_EFFECT) {
						opponent->fighters[0].affect(effects[i][2]);
						announce(CRIPPLE_ANNOUNCEMENT);
					}
	...
	private:
		...
		int card_limit = BASE_CARD_LIMIT;
		int plays = BASE_CARD_LIMIT;
};
//}
...
const DeckCode SPLASH_DECK(
	"Splash",
	"This is a combo-oriented deck that aims to defeat "
