...
constexpr int PLAYER_EXPLANATION_COUNT = 12;
...
#define PLAYER_EFFECT_CONDITIONS(X) (                      \
	...
	: X == 5 ? static_cast<bool>(BANISHMENT_CONDITION)     \
	: X == 6 ? static_cast<bool>(FREEDOM_CONDITION)        \
	: X == 7 ? static_cast<bool>(AGILITY_AURA_CONDITION)   \
	: X == 8 ? static_cast<bool>(HEAL_AURA_CONDITION)      \
	: X == 9 ? static_cast<bool>(PLAYER_POWER_CONDITION)   \
	: X == 10 ? static_cast<bool>(END_DRAW_CONDITION)      \
...
#define PLAYER_EFFECT_VALUES(X) (   \
	...
	: X == 5 ? BANISHMENT_VALUE     \
	: X == 6 ? FREEDOM_VALUE        \
	: X == 7 ? AGILITY_AURA_VALUE   \
	: X == 8 ? HEAL_AURA_VALUE      \
	: X == 9 ? PLAYER_POWER_VALUE   \
	: X == 10 ? END_DRAW_VALUE      \
	: END_DISCARD_VALUE             \
)
//}
...
#define PLAYER_EFFECT_EXPLANATIONS(X) (   \
	...
	: X == 5 ? BANISHMENT_EXPLANATION     \
	: X == 6 ? FREEDOM_EXPLANATION        \
	: X == 7 ? AGILITY_AURA_EXPLANATION   \
	: X == 8 ? HEAL_AURA_EXPLANATION      \
	: X == 9 ? PLAYER_POWER_EXPLANATION   \
	: X == 10 ? END_DRAW_EXPLANATION      \
...
constexpr const char* NINJA_ATTACK_DESCRIPTION =
	"Flip 6 coins.\n"
	"For each heads, deal 200 damage to one of your opponent's fighters for each heads."
;
...
constexpr const char* SAMURAI_ATTACK_DESCRIPTION =
	"Deal 450 damage to your opponent's active fighter.\n"
	"Flip 2 coins.\n"
	"If both of them are heads, the opponent's active fighter "
	"can't attack or retreat during your opponent's next turn."
;
...
constexpr const char* FIRE_ELEMENTAL_ATTACK_NAME = "Wildfire";
constexpr const char* FIRE_ELEMENTAL_ATTACK_DESCRIPTION =
	"Deal 50 damage to all fighters.\n"
	"Deal 250 damage to your opponent's active fighter."
;
...
constexpr int FIRE_ELEMENTAL_ATTACK_DAMAGE = 350;
...
constexpr const char* WATER_ELEMENTAL_ATTACK_DESCRIPTION =
	"Heal 50 damage from all fighters.\n"
	"Deal 250 damage to your opponent's active fighter."
;
...
constexpr const char* EARTH_ELEMENTAL_ATTACK_NAME = "Crumble";
...
constexpr int EARTH_ELEMENTAL_ATTACK_DAMAGE = 250;
...
constexpr const char* DOCTOR_DESCRIPTION =
	"Discard your hand.\n"
	"Heal 750 damage from each of your fighters."
;
				...
				// Heals a fighter.
					...
					if (effects[i][1] == SELF_EFFECT) {
						if (effects[i][2] == DAMAGE_EFFECT) {
							int healing = last_draws * std::stod(effects[i][3]);
							int index = 0;
							healing = fighters[index].heal(healing);
							announce(HEAL_ANNOUNCEMENT);
						}
